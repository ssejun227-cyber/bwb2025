 ...
 * (영어 페이지용 - 이미지 경로 보정 포함)
 */

/**
 * 이미지 경로 보정 (assets/images/... → ../../assets/images/...)
 * 영어 페이지는 pages/en/ 하위에 있으므로 루트의 assets에 접근하려면 ../../ 필요
 */
function fixImagePath(path) {
  if (!path) return path;
  // 이미 절대경로거나 http로 시작하면 그대로
...
function renderHero(hero) {
  ...
  if (slider && hero.slides) {
    ...
    hero.slides.forEach((slide, index) => {
      ...
      div.style.backgroundImage = `url('${fixImagePath(slide)}')`;
...
function renderLinks(links) {
  ...
  if (mobileMenuList) {
    // 기존 동적 메뉴 제거 (중복 방지)
    mobileMenuList.querySelectorAll('.mobile-menu__dynamic').forEach(el => el.remove());
    
    links.forEach(link => {
      const li = document.createElement('li');
      li.className = 'mobile-menu__dynamic';
      li.innerHTML = `
        <a href="${link.url}" class="mobile-menu__link"${link.url.startsWith('http') ? ' target="_blank"' : ''}>
          <span class="mobile-menu__link-title">${link.text}</span>
        </a>
      `;
    ...
    switch (about.media.type) {
      case 'image':
        if (about.media.image) {
          mediaContainer.innerHTML = `<img src="${fixImagePath(about.media.image)}" alt="About" class="about__image">`;
        }
        break;
      case 'video':
        if (about.media.video) {
          mediaContainer.innerHTML = `
            <video class="about__video" controls>
              <source src="${fixImagePath(about.media.video)}" type="video/mp4">
            </video>`;
        }
        break;
      case 'youtube':
      ...
      row1Html += `
        <div class="hosts__group">
          ...
          <div class="hosts__logos">
            ${hosts.host.map(h => `<img src="${fixImagePath(h.logo)}" alt="${h.name}" class="hosts__logo">`).join('')}
          </div>
        </div>
      `;
    }
    ...
    if (hasOrganizer) {
      row1Html += `
        <div class="hosts__group">
          ...
          <div class="hosts__logos">
            ${hosts.organizer.map(o => `<img src="${fixImagePath(o.logo)}" alt="${o.name}" class="hosts__logo">`).join('')}
          </div>
        </div>
      `;
    }
    ...
    container.innerHTML += `
      <div class="hosts__row">
        <div class="hosts__group hosts__group--full">
          ...
          <div class="hosts__logos">
            ${hosts.support.map(s => `<img src="${fixImagePath(s.logo)}" alt="${s.name}" class="hosts__logo">`).join('')}
          </div>
        </div>
      </div>
    `;
    ...
    if (group.logos && group.logos.length > 0) {
      logosHtml = `
        <div class="org__logos">
          ...
          <div class="logo-grid">
            ${group.logos.map(l => `<img src="${fixImagePath(l.logo)}" alt="${l.name}" class="logo-grid__item">`).join('')}
          </div>
        </div>
      `;
    }
      ...
      <div class="org__group">
        ...
        <div class="grid grid--4cols">
          ${group.members.map(member => {
            ...
            return `
            <div class="card">
              <img src="${fixImagePath(member.photo)}" alt="${member.name}" class="card__image">
  ...
  if (sponsor.logos && sponsor.logos.length > 0) {
    container.innerHTML += `
      <div class="sponsor__logos">
        ...
        <div class="logo-grid">
          ${sponsor.logos.map(l => `<img src="${fixImagePath(l.logo)}" alt="${l.name}" class="logo-grid__item">`).join('')}
        </div>
      </div>
    `;
  }
...
function renderProgram(programData) {
  ...
  let html = `
    <div class="program__header">
      ...
      ${programData.venue ? `<p class="program__venue">Venue : ${programData.venue}</p>` : ''}
...
function renderFooter(footer) {
  ...
  if (items[0]) items[0].innerHTML = `<span class="footer__label">| Phone</span><span class="footer__value">${footer.phone}</span>`;
  if (items[1]) items[1].innerHTML = `<span class="footer__label">| Email</span><span class="footer__value">${footer.email}</span>`;
  if (items[2]) items[2].innerHTML = `<span class="footer__label">| Address</span><span class="footer__value">${footer.address}</span>`;
